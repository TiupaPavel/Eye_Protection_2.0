...
namespace Task_9
{
    partial class Form1
    {
        ...
        private void InitializeComponent()
        {
            ...
            // form6
            // 
            form6.BackColor = Color.Gray;
            form6.ClientSize = new Size(430, 287);
            form6.FormBorderStyle = FormBorderStyle.FixedSingle;
            form6.Icon = (Icon)resources.GetObject("form6.Icon");
            form6.Location = new Point(32, 32);
            form6.MaximizeBox = false;
            form6.MinimizeBox = false;
            form6.Name = "form6";
            form6.Text = "Notification";
            form6.Visible = false;
            // 
            // Form1
            // 
        ...
        private void SaveProfile(string filePath, string userName, string selectedColor, string lastSelectedTrack)
        {
            var profileData = $"{userName}\n{selectedColor}\n{lastSelectedTrack}";
            File.WriteAllText(filePath, profileData);
        }

        private (string userName, string selectedColor, string lastSelectedTrack) LoadProfile(string filePath)
        {
            var lines = File.ReadAllLines(filePath);

            string userName = lines.Length > 0 ? lines[0] : "";
            string selectedColor = lines.Length > 1 ? lines[1] : "1.0;1.0;1.0";
            string lastSelectedTrack = lines.Length > 2 ? lines[2] : "/";

            return (userName, selectedColor, lastSelectedTrack);
        }
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog.Title = "Select the profile to delete";
        ...
        private void saveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(lastProfilePath))
            {
                ...
                string selectedColor = $"{selectedRedGamma};{selectedGreenGamma};{selectedBlueGamma}";
                SaveProfile(lastProfilePath, userName, selectedColor, lastSelectedTrack);
            }
        }
        ...
        {
            OpenFileDialog openFileDialog = new OpenFileDialog();
            openFileDialog.Title = "Select a profile to open";
            openFileDialog.Filter = "Profile Files (*.profile)|*.profile";
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                var (userName, selectedColor, lastSelectedTrack) = LoadProfile(filePath);

                label6.Text = userName;

                var colorParts = selectedColor.Split(';');
                ...
                if (!string.IsNullOrEmpty(lastSelectedTrack))
                {
                    PlayTrack(lastSelectedTrack);
                }
            }
        }
        ...
        private void PlayTrack(string trackPath)
        {
            ...
            try
            {
                ...
                mp3Reader = new Mp3FileReader(trackPath);
        ...
        private void WaveOut_PlaybackStopped(object sender, StoppedEventArgs e)
        {
            ...
            {
                if (waveOut != null && mp3Reader != null && waveOut.PlaybackState != PlaybackState.Playing)
                {
        ...
        private string lastSelectedTrack = string.Empty;
        ...
        private void btnSelectTrack_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                lastSelectedTrack = openFileDialog.FileName;
                if (waveOut != null)
                {
                    waveOut.Stop();
                    waveOut.Dispose();
                ...
                mp3Reader = new Mp3FileReader(lastSelectedTrack);
                waveOut = new WaveOutEvent();
                waveOut.PlaybackStopped += WaveOut_PlaybackStopped;
                waveOut.Init(mp3Reader);
                waveOut.Play();
